...
test('should see discount on summary', async ({ page }) => {
  ...
  await page.locator('[data-test-id="catalog"] div').filter({ hasText: '$2.00/dayconvertiblePlaywright Hand2025 • Playwright TestRated by 13 people5.0$' }).getByRole('button').nth(3).click();
...
test('should not see discount on summary', async ({ page }) => {
  ...
  await page.locator('div').filter({ hasText: /^Pickup Date$/ }).getByRole('textbox').fill('2025-05-05');
  await page.locator('div').filter({ hasText: /^Return Date$/ }).getByRole('textbox').fill('2025-05-09');
  await expect(page.getByRole('main')).not.toContainText('$8.50');
  await expect(page.getByRole('main')).not.toContainText('Loyalty Discount (15%):');
});