...
test("User want to change valid date", async ({ page }) => {
   ...
   // await page.getByLabel('Close').click();
    ...
    await page.locator('div').filter({ hasText: /^Start Date$/ }).getByRole('textbox').fill('2027-09-09');
    await page.getByRole('button', { name: /^Save/i }).click();
    await expect(page.getByText('Reservation updated successfully')).toBeVisible({timeout:100000});
});
