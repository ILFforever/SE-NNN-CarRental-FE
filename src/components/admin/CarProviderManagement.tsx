...
export default function CarProvider({ token }: CarProviderProps) {
  ...
  const handleDeactivateProvider = async (providerId: string) => {
    if (
      !window.confirm(
        "WARNING: Deactivating this car provider will permanently delete ALL cars associated with this provider. Are you sure you want to continue?"
      )
    ) {
      return;
    }
  ...
  const handleToggleVerified = async (provider: CarProviderData) => {
    ...
    const updatedVerifiedStatus = !provider.verified;
